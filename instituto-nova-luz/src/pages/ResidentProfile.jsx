...
import defaultPhoto from '../assets/defaultProfile.jpg'
                ...
                <div className={styles.container}>
                    <div className={styles.personalInfo}>
                        <div className={styles.profilePhoto}>
                            {residente.imageUrl ? (<img src={residente.imageUrl} alt={residente.name} />) : (<img src={defaultPhoto} alt={residente.name} />)}
                        </div>
                        <div className={styles.profileInfo}>

                            {residente.name && <h3>{residente.name}</h3>}
                    ...
                    <div className={styles.obs}>
                        ...
                        {residente.obs && <p><span>Observações: </span>{residente.obs}</p>}
                    </div>
                    <div className={styles.codeContainer}>
                        <RecentCard code={residente.id} />
                    </div>
                    ...
                    <div className={styles.actions}>
                        <span onClick={() => navigate(`/dashboard/residente/${id}/perfil/editar`)} className={styles.yelowBtn}><GrDocumentUpdate /> Atualizar dados <span></span></span>
                        {!residente.exitPlug && <span onClick={() => navigate(`/dashboard/residente/${id}/ficha-de-saida`)} className={styles.yelowBtn}><MdDriveFolderUpload /> Ficha de saída <span></span></span>}
                        {residente.exitPlug && <span onClick={() => handleShowModal("Os dados da ficha de saída do residente serão excluidos", "Reativar vinculo?", "reativar")} className={styles.yelowBtn}><RiHeartAdd2Line /> Reativar vínculo <span></span></span>}
                        <span className={styles.download}><ResidentPdf residente={residente} /></span>
                        <span onClick={() => handleShowModal("Todos os dados do residente serão peridos e nao há como recuperá-los", "Atenção", "excluir")} className={styles.redBtn}><BiTrash /> Excluir residente<span></span></span>
                    </div>
